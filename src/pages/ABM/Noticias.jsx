  ...
  return (
    <div className="flex flex-col gap-10 items-center justify-center mt-10">
      ...
      {empresas.map((el, index) => (
        <Link to={`${el.id}/`} state={el.Denominación}  className="text-black bg-green-900 text-white w-[30%] self-center p-5 rounded-3xl cursor-pointer text-center text-2xl "><button key={index}>{el.Denominación}</button></Link>
      ))}
    </div>
  );
};
